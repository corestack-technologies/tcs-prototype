...
import { admitted, assigned } from "../../groups/model"
...
function fractionLabel(f: Fraction) {
  return f === 1 ? "Individual" : "Shared"
}

const posStatusConfig = {
  empty: {
    label: "Empty",
    border: "border-dashed border-[#E2E6F0]",
    bg: "bg-white",
    numColor: "text-[#9CA3AF]",
    badge: "bg-[#F1F3F8] text-[#9CA3AF]",
  },
  partial: {
    label: "Partial",
...
export function GroupPositions({ navigate }: Props) {
  ...
  const members: ApprovedMember[] = admitted(cycle).map((m) => ({
    ...m,
    equivCommitted: m.equivalent,
    assignedPositions: assigned(cycle, m.id).map((p) => p.position),
    equivAssigned: assigned(cycle, m.id).reduce((s, h) => s + h.fraction, 0),
  }))
  ...
  const assign = (posN: number, memberId: string, fraction: Fraction) => {
    ...
    if (newTotal > 1) {
      setError(
        `Adding a ${fractionLabel(fraction)} position to position ${posN} would exceed its capacity (${(totalCommitment(pos) * 100).toFixed(0)}% already assigned).`,
      )
      return
    }
    ...
    if (newEquiv > member.equivCommitted) {
      setError(
        `${member.name} is being assigned more than their committed equivalent positions (committed: ${member.equivCommitted}).`,
      )
      return
    }

    if (act({ type: "assign", position: posN, id: memberId, fraction }))
      setSelected(null)
  }

  const remove = (posN: number, memberId: string) => {
    act({ type: "unassign", position: posN, id: memberId })
    setError(null)
  }

  const selPos =
    selected !== null ? positions.find((p) => p.n === selected) : null
  const selRemaining = selPos ? 1 - totalCommitment(selPos) : 0
  const effectiveFraction: Fraction =
    GROUP.allowSplit && selRemaining === 0.5 ? 0.5 : assignFraction

  return (
    <OwnerShell navigate={navigate} activeView="owner-groups"><OwnerProgress current={2}/>
      <div className="px-6 py-3 bg-white border-b flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm">
          Payout order:{" "}
          {cycle.orderFinalized
            ? "Finalized"
            : "Proposed · changes require affected Members to reconfirm"}
        </p>
        <Button
          size="sm"
          disabled={!allComplete || cycle.orderFinalized}
          onClick={() => act({ type: "finalize" })}
        >
      ...
      <div className="bg-white border-b border-[#E2E6F0] px-6 lg:px-8 py-3.5 flex items-center gap-3 shrink-0">
        <button
          onClick={() => navigate("owner-group-recruit")}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button
            ...
            onClick={() => navigate("owner-group-rules", undefined)}
          >
            Continue →
          </Button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 lg:px-8 py-6">
        <div className="max-w-5xl mx-auto flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              },
              {
                ...
                value:
                  ...
                  (GROUP.positions -
                    ...
                    overPositions),
                color:
                  partialPositions > 0 ? "text-[#D97706]" : "text-[#9CA3AF]",
              },
              {
                label: "Over-allocated",
                value: overPositions,
                color: overPositions > 0 ? "text-[#DC2626]" : "text-[#9CA3AF]",
              },
            ].map((s) => (
              <div
                key={s.label}
              ...
              >
                ...
                <p className={`display-font text-2xl font-bold ${s.color}`}>
                  {s.value}
                </p>
              </div>
            ))}
          ...
          {allComplete && (
            <Alert type="success">
              All positions fully allocated. You can continue to rules review.
            </Alert>
          )}
          {partialPositions > 0 && !error && (
            <div className="bg-[#FFFBEB] border border-[#FDE68A] rounded-xl px-5 py-3 text-sm text-[#92400E]">
              <span className="font-semibold">
                ...
                {partialPositions !== 1 ? "s are" : " is"} partially filled.
              </span>{" "}
              A position is complete only when its total assigned commitment
              equals one full position. You cannot launch with partially filled
              positions.
            </div>
          )}
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border-2 p-4 transition-all cursor-pointer ${
                        isSelected
                          ? "border-[#1746A2] bg-[#EEF2FF] shadow-md"
                          : `${cfg.border} ${cfg.bg} hover:opacity-90`
                      }`}
                      role="group"
                      aria-label={"Payout position " + pos.n}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <button
                          type="button"
                          aria-label={"Assign payout position " + pos.n}
                          disabled={!canAdd}
                          onClick={() =>
                            setSelected((n) => (n === pos.n ? null : pos.n))
                          }
                          className={`display-font text-xl font-black ${
                            isSelected ? "text-[#1746A2]" : cfg.numColor
                          }`}
                        >
                          #{pos.n}
                        </button>
                      ...
                      {pos.holders.length > 0 ? (
                        <div className="flex flex-col gap-1.5 mb-2">
                          {pos.holders.map((h) => {
                            ...
                            return (
                              ...
                              >
                                <div className="w-5 h-5 rounded-full bg-[#1746A2] text-white text-[9px] font-bold flex items-center justify-center shrink-0">
                                  {m.initials}
                                </div>
                                <p className="text-xs font-semibold text-[#0D1117] flex-1 truncate">
                                  {m.name}
                                </p>
                                <span className="text-[10px] font-bold text-[#6B7280] shrink-0">
                                  {fractionLabel(h.fraction)}
                                </span>
                                <button
                                  aria-label={
                                    "Unassign " +
                        ...
                        <p className="text-xs text-[#9CA3AF]">
                          {isSelected
                            ? "Select a member →"
                            : "Select the position number to assign"}
                        </p>
                      )}
                      ...
                      <div className="h-1 bg-white/60 rounded-full overflow-hidden mt-2">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, totalCommitment(pos) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
            ...
            <div className="sticky top-6">
              ...
              {selected &&
                ...
                selRemaining < 1 && (
                  <div className="bg-[#EEF2FF] border border-[#C7D2FE] rounded-xl px-4 py-3 mb-3 text-xs text-[#374151]">
                    ...
                    Shared position — only a shared-position assignment is valid
                    here.
                  </div>
                )}
              ...
              <div className="bg-white rounded-xl border border-[#E2E6F0] overflow-hidden">
                <div className="divide-y divide-[#F1F3F8]">
                  {members.map((m) => {
                    ...
                    const alreadyInPos =
                      selPos?.holders.some((h) => h.memberId === m.id) ?? false
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          canAssign &&
                          selected &&
                          assign(selected, m.id, effectiveFraction)
                        }
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-[#9CA3AF]">
                            {m.equivAssigned === 0
                              ? "Not yet assigned"
                              : `Pos. ${m.assignedPositions.join(", ")} · ${m.equivAssigned} equiv.`}
                          </p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="text-[10px] font-bold text-[#6B7280]">
                            {m.equivAssigned}/{m.equivCommitted}
                          </p>
                          {canAssign && (
                ...
                {selected && (
                  <div className="px-4 py-3 border-t border-[#F1F3F8] bg-[#EEF2FF] flex items-center justify-between">
                    <p className="text-xs font-semibold text-[#1746A2]">
                      Position #{selected} · Assigning{" "}
                      {effectiveFraction === 1 ? "individual" : "shared"}{" "}
                      position
                    </p>
                    <button
                      onClick={() => {
                      ...
                      className="text-xs text-[#9CA3AF] hover:text-[#0D1117]"
                    >
                      Cancel
                    </button>
                  </div>
              ...
              <div className="mt-3 bg-[#F8FAFF] rounded-xl border border-[#E2E6F0] px-4 py-3 text-xs text-[#6B7280] leading-relaxed">
                <p className="font-semibold text-[#0D1117] mb-1">
                  How positions are completed
                </p>
                A position is complete only when its total assigned commitment
                equals one full position. A shared position requires all
                participant slots to be filled. Members can be assigned to
                multiple positions up to their approved commitment.
              </div>
            </div>
          </div>
        </div>
