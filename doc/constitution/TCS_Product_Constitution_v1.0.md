...
**Constitution v1.0 — TCS Prototype v1 freeze**
...
For MVP, an Organization has one Organization Owner. A verified, eligible Member may own one Organization and may separately participate in eligible Groups across Organizations. Organization approval and workspace activation are distinct.
...
Advance payment may be permitted and should normally allocate to the earliest eligible unpaid obligation unless explicit selection is allowed. A payment received before the eligible Round opens is held as an advance reservation, not a fabricated future obligation. Opening creates the scheduled obligation and releases valid reservations once. Ineligible or conflicting funds remain visible as exceptions.
...
A Member's own contribution component for the receiving Position in that Round is optional. Contributions for that Member's other Positions remain required. Split beneficiaries retain separate entitlements and recipient responses. The optional decision is finalized for the payout calculation; a later incompatible optional receipt is an exception rather than a silent rewrite of that calculation.
...
One appeal is permitted where applicable. A different authorized reviewer must handle the appeal; the original deciding reviewer cannot decide their own appealed outcome.
...
This document is **TCS Product Constitution v1.0**, the frozen product rule baseline for production handoff. Baseline v0.3 is retained unchanged for history. Product discovery/prototyping is closed; production-discovered corrections require deliberate review and traceable amendment.
...
# v1.0 Final Governance and Operating Rules

The following provisions complete Chapters 24, 28 and 29 and resolve the final operational rules. They are product rules, not interface or database design.

## C34 — Exact authority and separation of duties

Internal authority is default-deny and derives from the active user's currently active roles and exact recognized permissions. Unknown permissions, inactive roles, suspended/deactivated users and locked accounts confer no authority. Viewing a module does not imply authority to decide its cases. Access administration does not implicitly confer Operations or Settings authority; Business Date authority does not imply process execution.

An administrator must not increase their own effective authority through role assignment or role editing, including indirect changes. Protections preserve the last effective Access Administrator. Privileged changes require a reason, current authorization and a reviewable before/after record. Historical decisions retain the deciding authority even after later role changes.

The Organization Owner may not approve a directly beneficial exception. Appeals require an independent reviewer. Force Close and other serious lifecycle actions preserve their specific initiation, evidence, consent and approval guards; a broad role name is not a bypass.

## C35 — Source decisions and operational oversight

Operations cases refer to source business records. Assignment, notes and queue status do not themselves confirm payment, resolve Recovery or change entitlement. A permitted decision validates current source state and records the outcome, actor, reason, evidence and prior state. A stale preview or changed authority must be re-evaluated. Dispute findings distinguish full receipt, partial receipt and non-receipt; original postings remain intact and any required financial adjustment is separately visible. TCS does not simulate reversal authority over an external bank transfer.

## C36 — Business Date and runtime configuration

The real-time business calendar uses Africa/Lagos. Controlled Business Date is exclusively a non-production UAT capability. Production uses real business time and must reject controlled-date changes. Actual event/audit timestamps remain distinct from effective business dates; changing Business Date never rewrites provider success timestamps or historical financial facts.

Date changes require explicit authority, a reason and a current impact preview. Rewind or return to real time cannot cross protected financial/source history. Advancing the date does not itself execute lifecycle processing. Contact verification mode may require Email, Phone or Both for subsequent onboarding eligibility; it is not a KYC approval override. Only explicitly supported runtime controls may be changed.

## C37 — Scheduled processing

Daily lifecycle evaluation executes in this order: Round/obligation generation and valid advance release; daily penalty accrual; payout lifecycle windows; Post-Payout Default evaluation; financial reconciliation and recognized revenue share. Domain rules remain authoritative.

Catch-up retains original schedule dates. Duplicate execution must not duplicate obligations, allocations, accruals, default events or financial postings. Partial failure preserves completed work and identifies failures; an authorized retry links to its original run and remains idempotent. Runs retain actual start/end, effective Business Date, environment, actor authority, reason, step results and counts. Controlled scenario clocks do not silently enter environment-clock processing.

## C38 — Reporting and financial source integrity

Reports are read-only projections of authorized source records. Members see their own records across eligible Organizations; Owners see their Organization's scope; Internal users require the exact report permission and any protected source permission. Other Organizations receive eligibility indicators, not private cross-Organization debt details.

Reports distinguish required principal, optional contributions, accrued and waived penalties, Recovery allocations, recognized contribution, provider settlement, payout transfer, evidenced receipt, Organization Fee and TCS Revenue Share. Linked Recovery and penalty amounts must not be counted twice. Financial measures of different source grains are not one additive cash balance. Missing facts remain unavailable rather than invented zeros or dates. Filters, date basis and exports preserve scope and source traceability.

## v0.3 review and v1.0 disposition

Chapters 1–33 remain the reviewed foundation, including the approved daily penalty rules in Chapter 13 and seven-full-local-calendar-day threshold in §15.3. v1.0 clarifies one-Owner ownership, receiving-Position optionality, advance reservations, independent appeals and final governance/time/process/reporting provisions C34–C38. It does not invent a new rate, payment rail, commercial waiver, staffing model or production integration.

The 10 bps/day ceiling is an approved prototype policy value requiring production review, not an approved regulatory limit. Historical demo rates and aggregate-only examples do not establish new commercial rules. Production integrations, legal/provider validation and persistent operational infrastructure remain prerequisites to live operation. Features such as closure/succession controls express approved obligations even where the prototype uses simplified clearance records; no claim of complete live infrastructure follows from this Constitution.
